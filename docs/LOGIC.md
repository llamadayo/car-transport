...
**Step 4 全部班次皆失敗 → 分兩種原因（不留候補、不排隔日）**
...
### 派車主檔常數
- **出發（基地）據點 `DB.homeSite`**：主檔參數，不寫死；去/回程方向與行經序列由其在南北順序中的位置推算。
- 相鄰據點行駛 `legMinutes = 55 分`；出發示意 08:00。
- **時間上限動態決定**（天數為整台車屬性）：`timeLimitFor(終點, 直達?) = 查表天數 × workdayMin`；直達查 `dayCountDirect`、非直達查 `dayCountStopover`；查無 → 全域上限 `maxTripDays`（3 天）保險。
- 容量基準＝**有效體積 `effVol`**（Level 1 + 形狀，不做 Level 2）＋重量。

### 去程派車 `dispatch(vehicleId, mode)`
僅處理 `approved` 且**南下**（由起迄推導）的單，依 `approvedAt` 排序。
...
1. **行駛**：`totalTime += |前站-本站|×55`；若 `> timeLimitFor(本站, 非直達)` → 終點不再延伸。
...
3. **再裝貨**：本站為 `pickSite` 的 approved 單，依核准序，**整張表單為最小單位**：
   ...
   - **容量/時間**：`netVol+eff ≤ 車容量` 且 `netWt+weight ≤ 車重` 且 `totalTime+loadMin ≤ 本站時間上限` → 裝入(`loaded`)，記 `pickupTime`；否則整張跳過（留下一班）。
...
- **空車移動最小化（最高優化目標）**：空駛 `deadheadMin = |當前位置 − 出發地據點| × legMinutes`；候選清單依**空駛總和升冪排序**，取第一個可行者 → 空駛最小優先、車次數次要。**取捨權重待業務確認**。
...
flowchart TD
  ...
  A1 --> A2{"排序班次<br/>asap：最早出發 / exact：|到站−期望| 最小（早晚都比）"}
  A2 --> A3{"逐班次嘗試"}
  A3 --> C2{"站內時間額度<br/>上貨計收貨站、卸貨計送貨站<br/>各站累計 ≤ 40 分？"}
  C2 -- 否，順延下一班 --> A3
...
| 時間條件 | 站內額度 40 分（上貨/卸貨各站計） | 行駛+裝卸 ≤ **查表天數×工時** + 交貨時間門檻 | 工時 ≤ 20:30 + 單程 4 小時窗 |
| 期望時間 | **僅排序**，回報時間差、不退件 | 交貨時間為**門檻**（晚到留下一班） | 上車時間為合併比對條件 |
| 失敗處理 | toobig / full（不留候補） | 留下一班 / 自動順延 | 待人工協調（可人工指派）/ 逾期作廢 |

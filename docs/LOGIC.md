...
### 司機任務單
依 **`serviceDate` ＋ `assignedShift`** 分組，每個「日期＋班次（車輛）」一張任務單（不同日期不混在同一張）。車輛沿固定 10 站路線**一次通過**，故任務單**以停靠站為單位、依站序（S1→S10）排列**：為每張單建立「取貨（收貨站）」與「卸貨（送貨站）」兩個停靠事件，同一站的收/送**自動彙整**（先卸後裝），每站顯示抵達時間、卸貨清單（單號/送貨地點/接收人/狀態）與取貨清單（單號/收貨地點/貨物）。抵達時間走 `shiftArrivalAtStation(班次, 站序)`（發車＋站序×12 分，示意）。
...
- 抵達目的地 `directEta = 表定 08:00 + 前置緩衝 + travelMin(出發, 目的地) + 本趟各取貨據點上貨時間總和`（查路程表）。取貨據點必落在「基地→目的地」南下路徑上，故行駛仍以直達時間計，另計沿途上貨時間，避免低估抵達。
- **交貨時間門檻**：`directEta > 交貨時間` → 該單留下一班直達車。
- **各單來收時間** `pickupTime = 表定 08:00 + 前置緩衝 + travelMin(出發, 該單 pickSite) + 先前取貨點上貨時間`（依 `pickSite` 由北而南計算，即車輛經過該取貨據點的時間，非抵達迄點時間）。
